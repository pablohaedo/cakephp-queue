...
namespace Queue\Test\TestCase\Model\MongoCollection;

use Cake\I18n\Time;
use Cake\TestSuite\TestCase;

use Queue\Model\Entity\MongoQueuedJob;
use Queue\Model\MongoCollection\QueuedJobsCollection;

use CakeMonga\MongoCollection\CollectionRegistry;

/**
 * Queue\Model\MongoCollection\QueuedJobsCollection Test Case
 */
class QueuedJobsCollectionTest extends TestCase {

	/**
	 * @var \Queue\Model\MongoCollection\QueuedJobsCollection
	 */
	 ...
	 */
	public $fixtures = [
		'plugin.queue.QueuedJobsCollection',
	];
	...
	public function testQueueInstance() {
		$this->assertInstanceOf(QueuedJobsCollection::class, $this->QueuedJobs);
	}
	...
	public function testCreateAndFetch() {
		...
		$this->assertEquals('task1', $job->getJobType());
		$this->assertEquals(0, $job->getFailed());
		$this->assertNull($job->getCompleted());
		$this->assertEquals($testData, json_decode($job->getData(), true));
	...
	public function testSequence() {
		...
		foreach (range(0, 4) as $num) {
			...
			$jobData = json_decode($array[$num]->getData(), true);
	...
	public function testNotBefore() {
		...
		$data = iterator_to_array($this->QueuedJobs->find());
		$keys = array_keys($data);
		$this->assertWithinRange((new Time('+ 1 Min'))->toUnixString(), MongoQueuedJob::fromArray($data[$keys[0]])->getNotBefore(), 60);
		$this->assertWithinRange((new Time('+ 1 Day'))->toUnixString(), MongoQueuedJob::fromArray($data[$keys[1]])->getNotBefore(), 60);
		$this->assertWithinRange((new Time('2009-07-01 12:00:00'))->toUnixString(), MongoQueuedJob::fromArray($data[$keys[2]])->getNotBefore(), 60);
	}
	...
	public function testNotBeforeOrder() {
		...
		foreach ($expected as $item) {
			...
			$this->assertEquals($item['name'], $tmp->getJobType());
			$this->assertEquals($item['data'], json_decode($tmp->getData(), true));
			break;
		}
	}
	...
	public function testRequestGroup() {
		...
		$progress = iterator_to_array($this->QueuedJobs->find(['job_group' => 'testgroup']));

		$this->assertEquals(3, count($progress));
		$keys = array_keys($progress);

		$this->assertNull($progress[$keys[0]]['reference']);
		$this->assertEquals('Job number 4', $progress[$keys[1]]['reference']);
		$this->assertEquals('Job number 6', $progress[$keys[2]]['reference']);
	}
